  ...
  return (
    <div className="md:hidden fixed bottom-0 left-0 right-0 bg-white dark:bg-[var(--mongodb-navy)] border-t border-gray-200 dark:border-gray-800 z-50">
      <div className="flex justify-around items-center h-16 px-4">
        ...
        <Link href="/projects" className="flex flex-col items-center justify-center">
          <div className={`p-2 rounded-full ${pathname === '/projects' ? 'bg-gray-100 dark:bg-gray-800' : ''}`}>
            <svg xmlns="http://www.w3.org/2000/svg" className={`h-6 w-6 ${pathname === '/projects' ? 'text-[var(--mongodb-dark-green)]' : 'text-gray-600 dark:text-gray-400'}`} fill="none" viewBox="0 0 24 24" stroke="currentColor">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z" />
            </svg>
          </div>
          <span className="text-xs mt-1 font-medium text-gray-600 dark:text-gray-400">Buscar</span>
        </Link>
